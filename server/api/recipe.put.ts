...
export default eventHandler(async (event) => {
  ...
  const updateRecipe = async (userId: string) => {
  ...
  const updateRecipesAlimentaryProducts = async () => {
  ...
  try {
    ...
    const recipeId = await updateRecipe(userId)
    await updateRecipesAlimentaryProducts()
    // await _postRecipesKitchenEquipments(recipeObject.id)
